...
let patternBlock = {
  width: 4,
  height: 4,
  tureBlockIdx: [5, 6, 9, 10],
};

let patternBlinker = {
  width: 5,
  height: 5,
  tureBlockIdx: [11, 12, 13],
};

let patternToad = {
  width: 6,
  height: 6,
  tureBlockIdx: [14, 15, 16, 19, 20, 21],
};

let patternGlider = {
  width: 5,
  height: 5,
  tureBlockIdx: [7, 13, 16, 17, 18],
};
...
let addPattern = (x, y, pattern) => {
  for (let row = 0; row < pattern.height; row++) {
    for (let col = 0; col < pattern.width; col++) {
      ...
      if (pattern.tureBlockIdx.includes(pattern.width * row + col)) {
